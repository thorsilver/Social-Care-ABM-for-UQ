...
class Sim:
        ...
        for person in self.pop.livingPeople:
            ...
            if random.random() < dieProb:
                ...
                if person.house == self.displayHouse:
                    messageString = str(self.year) + ": #" + str(person.id) + " died aged " + str(self.year - person.birthdate) + "." 
        ...
        for person in peopleNotInCriticalCare:
            ...
            if person.sex == 'male':
                ageCareProb = ( ( math.exp( age /
                               ...
                               * self.p['maleAgeCareProb'] )
            else:
                ageCareProb = ( ( math.exp( age /
                                           self.p['femaleAgeCareScaling'] ) )
                               * self.p['femaleAgeCareProb'] )
    ...
    def doBirths(self):
        ...
        for woman in womenOfReproductiveAge:
            ...
            if self.year > self.p['transitionYear']:
                birthProb = self.p['steadyPopBirthProb']
    ...
    def updateCanvas(self):
        ...
        xBasic = 580 + (self.displayHouse.town.x * self.p['pixelsPerTown'])
        yBasic = 15 + (self.displayHouse.town.y * self.p['pixelsPerTown'])
        xOffset = xBasic + 2 + (self.displayHouse.x * 2)
        yOffset = yBasic + 2 + (self.displayHouse.y * 2)

        self.canvas.create_line ( 300, 650, xOffset, yOffset,
                                  fill = outlineColour,
                                  tags='redraw')
    ...
    def doGraphs(self):
        ...
        pylab.plot(self.times,self.pops,color="red")
        pylab.xlim(xmin=self.p['statsCollectFrom'])
        pylab.ylabel('Population')
        pylab.xlabel('Year')
        pylab.savefig('popOnly.pdf')
        pylab.savefig('popOnly.png')
        pylab.show()
...
##        pylab.savefig('avgHousehold.png')
##        pylab.show()
...
##        pylab.savefig('numDivorces.pdf')
        ...
        pylab.plot(self.times,self.totalCareDemand,color="red")
        pylab.xlim(xmin=self.p['statsCollectFrom'])
        pylab.ylabel('Total care demand in hours')
        pylab.xlabel('Year')
        pylab.savefig('careDemandOnly.pdf')
        pylab.savefig('careDemandOnly.png')
        pylab.show()
...
##        pylab.savefig('informalCare.png')
##        pylab.show()
